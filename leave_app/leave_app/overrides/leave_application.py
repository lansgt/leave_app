...
class LeaveApplication(Document):
	...
	def validate_leave_overlap(self):
		...
		for d in frappe.db.sql(
			...
			if (
				...
				and (
					flt(self.total_leave_days) == flt(self.custom_hours)/8
	...
	def get_total_leaves_on_half_day(self):
		...
		return leave_count_on_half_day_date * flt(self.custom_hours)/8
...
def get_number_of_leave_days(
	...
	custom_hours: Union[float, str, None] = 0,
	...
	if cint(half_day) == 1:
		if getdate(from_date) == getdate(to_date):
			number_of_days = flt(custom_hours)/8
		elif half_day_date and getdate(from_date) <= getdate(half_day_date) <= getdate(to_date):
			number_of_days = date_diff(to_date, from_date) + flt(custom_hours)/8
		else:
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if leave_entry.leaves % 1:
				...
				custom_hours = frappe.db.get_value(
					"Leave Application", {"name": leave_entry.transaction_name}, ["custom_hours"]
				)
