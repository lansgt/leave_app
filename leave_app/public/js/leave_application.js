...
frappe.ui.form.on("Leave Application", {
	...
	onload: function(frm) {
		...
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "leave_app.leave_app.overrides.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.employee) {
			frappe.call({
				method: "leave_app.leave_app.overrides.leave_application.get_leave_details",
				async: false,
				args: {
					employee: frm.doc.employee,
	...
	get_leave_balance: function(frm) {
		if (frm.doc.docstatus === 0 && frm.doc.employee && frm.doc.leave_type && frm.doc.from_date && frm.doc.to_date) {
			return frappe.call({
				method: "leave_app.leave_app.overrides.leave_application.get_leave_balance_on",
				args: {
					employee: frm.doc.employee,
	...
	calculate_total_days: function(frm) {
		if (frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			return frappe.call({
				method: 'leave_app.leave_app.overrides.leave_application.get_number_of_leave_days',
				args: {
					"employee": frm.doc.employee,
					"leave_type": frm.doc.leave_type,
					...
					"custom_hours": frm.doc.custom_hours,
				},
				callback: function(r) {
					if (r && r.message) {
	...
	set_leave_approver: function(frm) {
		if (frm.doc.employee) {
			// server call is done to include holidays in leave days calculations
			return frappe.call({
				method: 'leave_app.leave_app.overrides.leave_application.get_leave_approver',
				args: {
					"employee": frm.doc.employee,
				},
				callback: function(r) {
